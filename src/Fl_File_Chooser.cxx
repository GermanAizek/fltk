...
Fl_File_Chooser::Fl_File_Chooser(const char *pathname, const char *pattern, int type_val, const char *title)
  : fav_(nullptr), ext_(new ChooserExt())
{
  if (!prefs_) {
...
void Fl_File_Chooser::ensure_favWindow() {
  if (fav_) return;
  fav_ = new FavExt();
  Fl_Group *prev_current = Fl_Group::current();
  fav_->favWindow_ = new Fl_Double_Window(355, 150, "Manage Favorites");
  fav_->favWindow_->user_data((void*)(this));
  { fav_->favList_ = new Fl_File_Browser(10, 10, 300, 95);
    fav_->favList_->type(2);
    fav_->favList_->callback((Fl_Callback*)cb_favList);
    Fl_Group::current()->resizable(fav_->favList_);
  }
  { Fl_Group* o = new Fl_Group(320, 10, 25, 95);
    { fav_->favUpButton_ = new Fl_Button(320, 10, 25, 25, "@8>");
      fav_->favUpButton_->callback((Fl_Callback*)cb_favUpButton);
    }
    { fav_->favDeleteButton_ = new Fl_Button(320, 45, 25, 25, "X");
      fav_->favDeleteButton_->labelfont(1);
      fav_->favDeleteButton_->callback((Fl_Callback*)cb_favDeleteButton);
      Fl_Group::current()->resizable(fav_->favDeleteButton_);
    }
    { fav_->favDownButton_ = new Fl_Button(320, 80, 25, 25, "@2>");
      fav_->favDownButton_->callback((Fl_Callback*)cb_favDownButton);
    }
    o->end();
  }
  { Fl_Group* o = new Fl_Group(10, 113, 335, 29);
    { fav_->favCancelButton_ = new Fl_Button(273, 115, 72, 25, "Cancel");
      fav_->favCancelButton_->callback((Fl_Callback*)cb_favCancelButton);
      fav_->favCancelButton_->label(fl_cancel);
    }
    { fav_->favOkButton_ = new Fl_Return_Button(181, 115, 79, 25, "Save");
      fav_->favOkButton_->callback((Fl_Callback*)cb_favOkButton);
      fav_->favOkButton_->label(save_label);
    }
  ...
  fav_->favWindow_->set_modal();
  fav_->favWindow_->size_range(181, 150);
  fav_->favWindow_->label(manage_favorites_label);
  fav_->favWindow_->end();
  Fl_Group::current(prev_current);
}
