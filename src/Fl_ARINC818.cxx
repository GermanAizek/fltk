...
Fl_ARINC818::Fl_ARINC818()
  : arinc818_cb_(nullptr), user_data_(nullptr) {
  last_container_.frame_count = 0;
  last_container_.video_width = 0;
  last_container_.video_height = 0;
  last_container_.frame_rate_fps = 0;
  last_container_.ancillary_len = 0;
  last_container_.container_ver = 0;
  last_container_.color_format = 0;
  last_container_.is_valid = false;
...
void Fl_ARINC818::process_byte(uint8_t b) {
  if (buffer_.empty() && b != 0x81) return;
  ...
  if (buffer_.size() >= 16) {
    ...
    if (buffer_.size() == (size_t)(16 + anclen)) {
      feed_container(w, h, fps, fnum, buffer_.data() + 16, anclen);
      buffer_.clear();
    }
  }
}
...
void Fl_ARINC818::feed_container(uint16_t width, uint16_t height, uint16_t fps, uint32_t frame_num, const uint8_t* ancillary, uint16_t anc_len) {
  ...
  last_container_.ancillary_len = anc_len;

  if (ancillary && anc_len > 0) {
    last_container_.ancillary_data.assign(ancillary, ancillary + anc_len);
  } else {
    last_container_.ancillary_data.clear();
  }
