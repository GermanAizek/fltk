//
// FLTK NFC Windows Driver Implementation
//
...
#include <config.h>
#include "Fl_WinAPI_NFC_Driver.H"
#include <FL/Fl.H>
#include <stdio.h>
#include <string.h>
...
Fl_WinAPI_NFC_Driver::~Fl_WinAPI_NFC_Driver() {
  stop_target_detection();
}

void Fl_WinAPI_NFC_Driver::timer_cb(void* data) {
  Fl_WinAPI_NFC_Driver* self = (Fl_WinAPI_NFC_Driver*)data;
  if (!self->context_) return;
  
  SCARD_READERSTATEA rs;
  memset(&rs, 0, sizeof(rs));
  rs.szReader = "\\\\?PnP?\\Notification";
  rs.dwCurrentState = SCARD_STATE_UNAWARE;
  
  // Quick poll
  LONG rv = SCardGetStatusChangeA((SCARDCONTEXT)self->context_, 0, &rs, 1);
...
int Fl_WinAPI_NFC_Driver::start_target_detection() {
...
void Fl_WinAPI_NFC_Driver::stop_target_detection() {
  if (context_) {
