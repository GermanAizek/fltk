//
// Fl_WinAPI_Multimedia_Driver implementation for the Fast Light Tool Kit (FLTK).
//
...
#include "Fl_WinAPI_Multimedia_Driver.H"
#include <FL/Fl.H>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define WIN_MULTIMEDIA_TICK_INTERVAL 0.05

#if defined(_WIN32)
Fl_Multimedia_Driver* Fl_Multimedia_Driver::new_multimedia_driver(Fl_Multimedia *player) {
  return new Fl_WinAPI_Multimedia_Driver(player);
}
#endif

Fl_WinAPI_Multimedia_Driver::Fl_WinAPI_Multimedia_Driver(Fl_Multimedia *player)
...
Fl_WinAPI_Multimedia_Driver::~Fl_WinAPI_Multimedia_Driver() {
  is_playing_ = false;
  is_paused_ = false;
}

int Fl_WinAPI_Multimedia_Driver::set_source(const char *url_or_path) {
  stop();
...
int Fl_WinAPI_Multimedia_Driver::play() {
  ...
  Fl::add_timeout(WIN_MULTIMEDIA_TICK_INTERVAL, timer_cb, this);
  return 1;
}

void Fl_WinAPI_Multimedia_Driver::pause() {
...
void Fl_WinAPI_Multimedia_Driver::stop() {
  is_playing_ = false;
  is_paused_ = false;
  Fl::remove_timeout(timer_cb, this);
  position_ms_ = 0;
...
void Fl_WinAPI_Multimedia_Driver::set_position(int64_t position_ms) {
...
void Fl_WinAPI_Multimedia_Driver::set_volume(double vol) {
  if (vol < 0.0) vol = 0.0;
  if (vol > 1.0) vol = 1.0;
  volume_ = vol;
}

void Fl_WinAPI_Multimedia_Driver::set_muted(bool muted) {
  muted_ = muted;
}

void Fl_WinAPI_Multimedia_Driver::set_playback_rate(double rate) {
  if (rate <= 0.0) rate = 1.0;
  playback_rate_ = rate;
}

void Fl_WinAPI_Multimedia_Driver::timer_cb(void *data) {
  Fl_WinAPI_Multimedia_Driver *driver = (Fl_WinAPI_Multimedia_Driver*)data;
  if (driver) {
    driver->tick();
  }
}

void Fl_WinAPI_Multimedia_Driver::tick() {
  if (!is_playing_) return;

  int64_t step_ms = (int64_t)(WIN_MULTIMEDIA_TICK_INTERVAL * 1000.0 * playback_rate_);
  ...
  if (duration_ms_ > 0 && position_ms_ >= duration_ms_) {
    ...
    player_->set_position_internal(position_ms_);
    Fl::repeat_timeout(WIN_MULTIMEDIA_TICK_INTERVAL, timer_cb, this);
  }
}
